 ...
 * 1. Audio transcription via OpenAI Whisper API (backend)
 * 2. AI chat with ASHA Didi via OpenAI (backend)
 * 3. Text-to-speech playback via OpenAI TTS (backend)
 * 4. Emergency detection and logging
 */

import apiClient from '../lib/api';
import { useStore } from '../store/useStore';
import { normalizeLanguage, resolveResponseLanguage } from '../utils/language';
...
export async function transcribeAudio(
    ...
    if (language) {
        ...
        if (normalized !== 'auto') {
            formData.append('language', normalized);
        }
    }
...
export async function processVoiceInput(
    ...
    // Step 1: Transcribe audio (auto-detect Hindi/English when no forced language)
    const transcription = await transcribeAudio(audioBlob);
    const transcript = transcription.transcript;
    const detectedLanguage = normalizeLanguage(
        transcription.language || resolveResponseLanguage(transcript, preferredLanguage),
        'hi',
    ) as 'hi' | 'en';
