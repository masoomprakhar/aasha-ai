  ...
  if (!profile || !profile.lastPeriodDate) {
    return (
      <RoleLayout role="beneficiary" hideHeader={true}>
        <div className="min-h-screen bg-canvas p-6 flex flex-col justify-center">
          <button onClick={() => navigate(-1)} className="absolute top-6 left-6 p-2 bg-white rounded-full shadow-sm">
            <ArrowLeft className="w-6 h-6 text-gray-800" />
          </button>
          
          <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 text-center">
            <div className="w-20 h-20 bg-brand-soft/40 rounded-full flex items-center justify-center mx-auto mb-6 text-brand">
              <Calendar size={40} />
            </div>
            <h1 className="text-2xl font-bold text-gray-900 mb-2">{t('dash.setup')}</h1>
            ...
            <input 
              type="date" 
              className="w-full p-4 bg-gray-50 rounded-xl border border-gray-200 font-bold text-gray-900 mb-6 focus:ring-2 focus:ring-rose-500 outline-none"
            ...
            <button 
              onClick={handleSetup}
              disabled={!setupDate}
              className="w-full py-4 bg-brand-dark text-white rounded-xl font-bold text-lg hover:bg-brand-dark disabled:opacity-50 disabled:cursor-not-allowed transition-all"
            >
              {t('common.submit')}
            </button>
          </div>
    ...
    <RoleLayout role="beneficiary" hideHeader={true}>
      <div className="min-h-screen bg-canvas p-4 pb-24">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <h1 className="text-lg font-bold text-gray-900">{t('tracker.title')}</h1>
          <div className="w-10" />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 relative overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-8 mt-2">
              <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest">{t('tracker.prediction')}</h3>
            ...
            <div className="text-center mb-8">
              <div className="flex items-baseline justify-center gap-2">
                <span className="text-7xl font-bold text-brand">{insights.daysToNextPeriod}</span>
                <span className="text-xl font-bold text-gray-400">days</span>
              </div>
              <p className="text-gray-500 font-medium mt-2">{t('dash.days_to_period')}</p>
              <p className="text-gray-900 font-bold mt-1">Expected: <span className="text-gray-900">{insights.nextPeriod}</span></p>
            </div>
          ...
          <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 flex flex-col items-center justify-center text-center relative overflow-hidden">
             <div className="absolute top-0 right-0 w-32 h-32 bg-brand-wash rounded-full blur-3xl -mr-10 -mt-10" />
             ...
             <h3 className="text-xl font-bold text-gray-900 mb-2 relative z-10">{t('tracker.voice_log')}</h3>
             <p className="text-gray-400 text-sm font-medium mb-6 max-w-[200px] relative z-10">
               {t('tracker.voice_desc')}
             </p>

             <button 
               onClick={() => setIsLogModalOpen(true)}
               className="w-full py-4 bg-gradient-to-r from-brand to-brand-dark text-white rounded-2xl font-bold text-lg shadow-lg shadow-brand-ring hover:scale-[1.02] active:scale-[0.98] transition-all relative z-10"
             >
               {t('tracker.tap_speak')}
             </button>
          </div>
          ...
          <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 flex flex-col">
            <div className="flex justify-between items-start mb-6">
              <div>
                <h3 className="text-lg font-bold text-gray-900">{t('tracker.health_log')}</h3>
                <p className="text-xs text-gray-400 font-bold">{format(selectedDate, 'MMMM d, yyyy')}</p>
              </div>
              <button 
                onClick={() => setIsLogModalOpen(true)}
                className="px-3 py-1 bg-brand-wash text-brand-dark rounded-lg text-xs font-bold flex items-center gap-1 hover:bg-brand-soft/40"
              >
            ...
            {currentLog ? (
              <div className="space-y-4 flex-1">
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-2xl border border-gray-100">
                  ...
                  <div>
                    <p className="text-xs font-bold text-gray-400 uppercase">{t('tracker.mood')}</p>
                    <p className="font-bold text-gray-900">{currentLog.mood}</p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {currentLog.symptoms.length > 0 ? (
                    currentLog.symptoms.map(s => (
                      <span key={s} className="px-3 py-1 bg-brand-wash text-brand-dark rounded-lg text-xs font-bold">
                        {s}
                      </span>
                    ))
                  ) : (
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center py-6">
                <p className="text-gray-400 font-medium text-sm">{t('tracker.no_logs')}</p>
                <button 
                  onClick={() => setIsLogModalOpen(true)}
