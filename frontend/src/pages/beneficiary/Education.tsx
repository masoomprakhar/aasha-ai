...
export default function BeneficiaryEducation() {
  ...
  return (
    <div className="min-h-screen bg-canvas pb-20">
      {/* Header */}
      <div className="bg-white dark:bg-gray-900 p-4 sticky top-0 z-20 shadow-sm">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h1 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
            <BookOpen size={20} className="text-brand" /> {t('edu.title')}
          </h1>
        </div>
        ...
        <div className="relative">
          ...
          <input 
            ...
            className="w-full pl-10 pr-4 py-3 bg-gray-100 dark:bg-gray-800 rounded-xl border-none focus:ring-2 focus:ring-brand outline-none"
            value={search}
      ...
      <div className="p-4 space-y-6">
        ...
        {activeCategory === 'all' && !search && featuredArticle && (
          <section>
            <h2 className="text-sm font-bold text-gray-500 uppercase mb-3 px-1">{t('edu.featured')}</h2>
            <div 
              onClick={() => setSelectedArticle(featuredArticle)}
              className={`p-6 rounded-3xl ${featuredArticle.color} relative overflow-hidden cursor-pointer group`}
            >
              <div className="relative z-10">
                <span className="inline-block px-3 py-1 bg-white/50 backdrop-blur-sm rounded-full text-[10px] font-bold uppercase mb-3 text-gray-800">
                  {featuredArticle.category}
                </span>
                <h3 className="text-2xl font-bold text-gray-900 mb-2 leading-tight max-w-[80%]">
                  {featuredArticle.title}
                </h3>
                <p className="text-gray-700 font-medium text-sm mb-6 max-w-[80%]">
                  {featuredArticle.subtitle}
                </p>
                <button className="px-5 py-2 bg-gray-900 text-white rounded-full text-xs font-bold flex items-center gap-2 group-hover:gap-3 transition-all">
              ...
              {/* Icon */}
              <div className="absolute bottom-4 right-4 w-16 h-16 bg-white/30 backdrop-blur-sm rounded-full flex items-center justify-center text-gray-900">
        ...
        {/* Article List */}
        <section>
          <h2 className="text-sm font-bold text-gray-500 uppercase mb-3 px-1">
          ...
          <div className="grid grid-cols-1 gap-4">
            {filteredArticles.length > 0 ? (
              filteredArticles.map(article => {
                ...
                return (
                  <GlassCard 
                    ...
                    className="p-4 flex items-center gap-4 hover:border-brand-soft group"
                  >
                    ...
                    <div className="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center group-hover:bg-brand-wash group-hover:text-brand transition-colors">
              ...
              <div className="text-center py-10">
                <p className="text-gray-400 font-medium">No articles found for your profile.</p>
              </div>
            )}
          </div>
        </section>
