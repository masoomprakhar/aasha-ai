...
export default function BeneficiarySchemes() {
  ...
  const SchemeCard = ({ scheme, enrolled = false }: { scheme: Scheme, enrolled?: boolean }) => {
    ...
    return (
      <div
        ...
        className={`bg-white p-6 rounded-3xl shadow-sm border flex flex-col md:flex-row gap-6 group transition-all cursor-pointer relative overflow-hidden ${enrolled ? 'border-green-200 bg-green-50/30' : 'border-gray-100 hover:shadow-md'
          }`}
      >
        <div className="w-full md:w-32 h-32 rounded-3xl bg-gray-50 shrink-0 overflow-hidden relative">
        ...
        <div className="flex-1 flex flex-col justify-center">
          <div className="flex justify-between items-start mb-2">
            <div>
              <div className="flex gap-2 mb-2">
                ...
                {enrolled ? (
                  ...
                  profile && (
                    <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-full flex items-center gap-1 ${eligibility.isEligible ? 'bg-brand-wash text-brand-dark' : 'bg-gray-100 text-gray-500'
                      }`}>
            ...
            {!enrolled && (
              <div className="w-10 h-10 rounded-full bg-panel flex items-center justify-center group-hover:bg-brand group-hover:text-white transition-colors">
          ...
          <div className="flex flex-wrap gap-2">
            {(scheme.benefits || []).slice(0, 2).map((benefit, i) => (
              <div key={i} className="flex items-center gap-1.5 text-xs font-bold text-gray-600 bg-gray-50 px-3 py-1.5 rounded-full">
  ...
  return (
    <div className="min-h-screen bg-canvas pb-24">
      <div className="bg-canvas/85 backdrop-blur-lg p-4 sticky top-0 z-20 border-b border-black/5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate(-1)} className="flex items-center justify-center w-10 h-10 rounded-full bg-panel text-gray-700 hover:text-gray-900 transition-colors">
            <ArrowLeft size={20} />
          </button>
          <h1 className="text-xl font-bold text-gray-900 tracking-tight">{t('schemes.title')}</h1>
        </div>
        <button className="flex items-center justify-center w-10 h-10 rounded-full bg-panel text-gray-700 hover:text-gray-900 transition-colors">
          <Search size={20} />
        </button>
      </div>
      ...
      <div className="p-6 max-w-3xl mx-auto space-y-8">
        ...
        {/* Hero Section */}
        <div className="bg-gradient-to-br from-brand-light to-brand p-8 rounded-3xl relative overflow-hidden shadow-lg text-white">
          <div className="relative z-10 max-w-xs">
            <p className="text-[11px] font-bold tracking-widest opacity-80 uppercase mb-3">{t('schemes.featured')}</p>
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
          {['all', 'financial', 'nutrition'].map((t) => (
            <button
              ...
              className={`px-6 py-2.5 rounded-full text-sm font-semibold capitalize transition-all whitespace-nowrap ${filter === t
                ? 'bg-brand text-white shadow-md shadow-brand-ring'
                : 'bg-white text-gray-500 border border-black/5 hover:bg-panel'
                }`}
            >
              {t}
            </button>
        ...
        <section>
          <h3 className="text-lg font-bold text-gray-900 mb-4 px-2">Available for You</h3>
          <div className="space-y-4">
            {availableSchemes.length > 0 ? (
              ...
              <div className="text-center py-12 bg-white rounded-3xl border border-dashed border-gray-200">
                <p className="text-gray-400 font-medium">No new schemes found matching your profile.</p>
              </div>
            )}
          </div>
        </section>
            ...
            >
              <div className="p-6 bg-gray-900 text-white flex justify-between items-start">
                <div>
                  ...
                  <p className="text-gray-400 text-sm mt-1">Eligibility Check</p>
                </div>
                <button
                ...
                {(() => {
                  ...
                  return (
                    <>
                      ...
                      <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider">{t('schemes.criteria_checklist')}</h3>
                          ...
                          <div key={idx} className="flex items-start gap-3 p-3 rounded-xl bg-gray-50 border border-gray-100">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="flex justify-between items-center mt-1">
                                <span className="text-xs text-gray-500">{t('schemes.required')}: {c.required}</span>
                      ...
                      <div className="mt-8">
                        {isEligible ? (
                          <button
                            ...
                            disabled={enrolling}
                            className="w-full py-4 bg-brand-dark text-white rounded-full font-semibold text-lg hover:bg-emerald-600 shadow-lg shadow-emerald-200 disabled:opacity-50 flex items-center justify-center gap-2"
                          >
                            {enrolling ? (
                              <>
                          ...
                          <button
                            ...
                            className="w-full py-4 bg-gray-100 text-gray-600 rounded-xl font-bold text-lg hover:bg-gray-200"
                          >
                            {t('dash.update')}
                          </button>
                        )}
