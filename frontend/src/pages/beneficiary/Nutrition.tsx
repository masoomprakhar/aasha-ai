...
export default function BeneficiaryNutrition() {
  ...
  return (
    <div className="min-h-screen bg-canvas pb-24">
      <div className="bg-white dark:bg-gray-900 p-4 sticky top-0 z-20 flex items-center gap-3 shadow-sm">
        <button onClick={() => navigate(-1)} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-full">
          <ArrowLeft className="w-6 h-6 text-gray-800 dark:text-white" />
        </button>
        <h1 className="text-xl font-bold text-gray-900 dark:text-white">{t('nutri.title')}</h1>
      </div>
      ...
      <div className="p-4 space-y-6 max-w-2xl mx-auto">
          ...
          <div className="relative z-10 flex justify-between items-center">
            ...
            <div className="flex flex-col items-end">
              <div className="bg-white/20 backdrop-blur-md rounded-2xl p-3 border border-white/20 text-center min-w-[100px]">
                ...
                {isAnemic && (
                  <span className="inline-block bg-white text-gray-900 text-[10px] font-bold px-2 py-0.5 rounded-full shadow-sm capitalize">
                    {theme.statusLabel}
                  </span>
                )}
              </div>
        ...
        <GlassCard className="p-2 flex items-center justify-between bg-white border-gray-100 shadow-sm">
          <button onClick={handlePrevDay} className="p-3 hover:bg-gray-100 rounded-full transition-colors text-gray-600">
            <ChevronLeft size={20} />
          </button>
          <div className="text-center">
            <h3 className="text-xl font-bold text-gray-900">{currentPlan.day}</h3>
            <p className="text-sm text-brand-dark font-bold">{currentPlan.localDay}</p>
          </div>
          <button onClick={handleNextDay} className="p-3 hover:bg-gray-100 rounded-full transition-colors text-gray-600">
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-bold text-gray-800 dark:text-white">{t('nutri.recommended')}</h3>
          </div>
            ...
            <AnimatePresence mode="wait">
              {currentPlan.meals.slice(0, 2).map((meal, idx) => {
                ...
                return (
                  ...
                  >
                    <GlassCard className="p-4 h-full flex flex-col justify-between bg-white border-gray-100 shadow-sm group hover:border-orange-200 hover:shadow-md transition-all">
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">{meal.type}</p>
                          <h4 className="font-bold text-gray-900 text-lg leading-tight mb-1">{meal.title}</h4>
                          <p className="text-gray-500 text-sm font-medium">{meal.subtitle}</p>
                        </div>
                      </div>
                      <div className="mt-4 flex flex-wrap gap-2">
                        {meal.tags.map(tag => (
                          <span key={tag} className="px-2 py-1 rounded-md bg-gray-100 text-gray-600 text-xs font-bold border border-gray-200">
                            {tag}
                          </span>
                        ))}
                      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-3xl p-6 border border-gray-800 shadow-lg relative overflow-hidden">
            ...
            <div className="flex flex-col items-center text-center relative z-10">
              ...
              <p className="text-gray-400 text-sm mb-6">Take after lunch with lemon water.</p>
          ...
          <GlassCard className="p-6 bg-white border-gray-100 shadow-sm">
            <h3 className="text-sm font-bold text-gray-400 uppercase mb-4 tracking-wider">{t('nutri.superfoods')}</h3>
            <div className="space-y-3">
              {SUPERFOODS.map(food => (
                <div key={food.id} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50 hover:bg-gray-100 transition-colors border border-gray-100">
                  <div className="text-2xl">{food.image}</div>
                  <div>
                    <h4 className="font-bold text-gray-900 text-sm">{food.name}</h4>
                    <p className="text-xs text-gray-500 font-medium">{food.desc}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-2xl flex items-center gap-4 border border-blue-100 dark:border-blue-800">
          ...
          <div>
             <h4 className="font-bold text-gray-800 dark:text-white text-sm">{t('nutri.hydration')}</h4>
             <p className="text-xs text-gray-600 dark:text-gray-400">{t('nutri.hydration_desc')}</p>
          </div>
        </div>
