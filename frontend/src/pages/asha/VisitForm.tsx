        ...
        {!selectedPatientId ? (
          <div className="space-y-6">
            <div className="text-center py-6">
              <div className="w-16 h-16 bg-brand-wash rounded-full flex items-center justify-center mx-auto mb-4">
                <User className="text-brand-dark w-8 h-8" />
              </div>
              <h2 className="text-xl font-bold text-gray-800">Select Patient</h2>
              <p className="text-gray-500 text-sm">Choose a beneficiary to record visit details</p>
            </div>
              ...
              <div className="space-y-2">
                {filteredBeneficiaries.map(b => (
                  <div
                    ...
                    className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm hover:border-emerald-500 hover:shadow-md transition-all cursor-pointer flex items-center justify-between group"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-semibold text-gray-800 group-hover:text-brand-dark">{b.name}</h3>
                        <p className="text-xs text-gray-500 capitalize">{b.userType} • {b.age}y</p>
                      </div>
                    </div>
                    <div className="px-3 py-1 bg-gray-100 rounded-full text-xs text-gray-600 group-hover:bg-brand-wash group-hover:text-brand-dark">
                      Select
                    </div>
                  </div>
                ))}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-brand-wash border border-emerald-100 p-4 rounded-xl flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center text-brand-dark font-bold border border-emerald-100">
                  {selectedPatient?.name.charAt(0)}
                </div>
                <div>
                  <p className="text-xs text-brand-dark font-bold uppercase">Recording for</p>
                  <h3 className="font-bold text-emerald-900">{selectedPatient?.name}</h3>
                </div>
              </div>
              <button
                onClick={clearSelection}
                className="p-2 bg-white text-gray-500 rounded-lg hover:text-red-500 hover:bg-red-50 transition-colors"
              >
                <RefreshCw size={18} />
              </button>
            </div>
