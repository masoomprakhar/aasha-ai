  ...
  return (
    <div className="min-h-screen bg-gray-900 flex flex-col relative">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-sm bg-gray-900 rounded-3xl overflow-hidden relative aspect-square flex items-center justify-center border border-white/10">
          ...
          {scanError && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-900/90 text-white p-6 text-center">
              ...
              <p className="text-sm text-gray-400 mb-6">{scanError}</p>
              <button 
                onClick={retryCamera}
                className="px-6 py-2 bg-emerald-600 rounded-full font-bold text-sm"
              >
                Retry Camera
              </button>
            </div>
        ...
        <div className="w-full max-w-xs mt-8">
          ...
          <form onSubmit={handleManualSubmit} className="relative">
            <input 
              ...
              className="w-full pl-4 pr-12 py-3 bg-white/10 border border-white/20 rounded-xl text-white placeholder:text-white/40 focus:outline-none focus:border-emerald-500 transition-colors"
              value={manualId}
              onChange={(e) => setManualId(e.target.value)}
            />
            <button 
              type="submit"
              className="absolute right-2 top-2 p-1.5 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors"
            >
              <Search size={16} />
            </button>
          </form>
