  ...
  return (
    <div className="min-h-screen bg-canvas">
      <div className="bg-white sticky top-0 z-10 border-b shadow-sm p-4 flex items-center gap-3">
        ...
        <h1 className="font-bold text-lg text-gray-800">{t('nav.alerts')}</h1>
      </div>

      <div className="p-4 space-y-6">
        <section>
          <h2 className="text-sm font-bold text-gray-500 uppercase mb-3 px-1">{t('asha.alerts_active')}</h2>
                      ...
                      <div className="mb-4">
                        ...
                        {patient?.gps_coords && (
                          <a 
                            ...
                            className="text-xs text-brand font-bold hover:underline flex items-center gap-1 mt-2 bg-brand-wash px-2 py-1 rounded w-fit"
                          >
                            View on Google Maps
                          </a>
                        )}
        ...
        {resolvedAlerts.length > 0 && (
          <section>
            <h2 className="text-sm font-bold text-gray-500 uppercase mb-3 px-1">Resolved History</h2>
            <div className="space-y-3 opacity-60">
              {resolvedAlerts.map(alert => {
                const patient = beneficiaries.find(b => b.id === alert.beneficiaryId);
                return (
                  <div key={alert.id} className="bg-white p-4 rounded-xl border border-gray-200 flex justify-between items-center">
                    <div>
                      <p className="font-bold text-gray-800">{patient?.name || 'Unknown'}</p>
                      <p className="text-xs text-gray-500">{formatDate(alert.timestamp)}</p>
                    </div>
                    <span className="px-2 py-1 bg-gray-100 text-gray-600 text-xs font-bold rounded flex items-center gap-1">
