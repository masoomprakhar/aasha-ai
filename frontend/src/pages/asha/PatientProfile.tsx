  ...
  if (!patient || !realId) return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-gray-50">
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-200 text-center">
        ...
        <p className="text-gray-500 mb-6">The patient ID you are looking for does not exist.</p>
    ...
    <div className="min-h-screen bg-canvas pb-20">
      ...
      <div className="bg-white p-4 sticky top-0 z-10 border-b shadow-sm flex items-center justify-between">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/asha')} className="p-2 hover:bg-gray-100 rounded-full">
            <ArrowLeft className="w-6 h-6 text-gray-600" />
          </button>
          <h1 className="font-bold text-lg text-gray-800">{t('asha.patient_profile')}</h1>
        </div>
      ...
      <main className="p-4 space-y-6">
          ...
          {editMode === 'basic' ? (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase">User Type</label>
                  <select
                ...
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase">Risk Level</label>
                  <select
              ...
              <div>
                <label className="text-xs font-bold text-gray-400 uppercase">Pregnancy Stage</label>
                <select
                  className="w-full p-2 border rounded-lg text-sm"
              ...
              <div>
                <button
                  onClick={handleGPS}
                  className="flex items-center gap-2 text-xs font-bold text-brand bg-brand-wash px-3 py-2 rounded-lg w-full justify-center"
                >
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-gray-500 text-sm capitalize flex items-center gap-1">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
            ...
            <p className="font-bold text-brand-dark">{patient.nextCheckup ? new Date(patient.nextCheckup).toLocaleDateString() : 'Not Scheduled'}</p>
          </div>
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
            <p className="text-xs text-gray-500 mb-1">Last Visit</p>
            <p className="font-bold text-gray-800">{patientLogs[patientLogs.length - 1] ? new Date(patientLogs[patientLogs.length - 1].date).toLocaleDateString() : 'Never'}</p>
          </div>
        </div>
        ...
        {activeTab === 'history' && (
          <section className="space-y-4">
            <Button onClick={() => navigate(`/asha/visit?patientId=${realId}`)} className="w-full bg-emerald-600 hover:bg-emerald-700">
            ...
            {patientLogs.length === 0 ? (
              <div className="text-center p-8 bg-white rounded-xl border border-dashed border-gray-300">
                <p className="text-gray-400 text-sm">No recent visits recorded.</p>
              </div>
            ) : (
                ...
                {patientLogs.map(log => (
                  <div key={log.id} className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm">
                    <div className="flex justify-between items-start mb-2">
                      <span className="text-xs font-bold text-gray-400">{new Date(log.date).toLocaleDateString()}</span>
                    ...
                    <div className="flex flex-wrap gap-1 mt-2">
                      {log.symptoms.map(s => (
                        <span key={s} className="text-[10px] px-2 py-1 bg-gray-100 text-gray-600 rounded">
                          {s}
                        </span>
                      ))}
                    </div>
        ...
        {activeTab === 'medical' && (
          <section className="space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="font-bold text-gray-800">Medical Details</h3>
              <button
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200 space-y-4">
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="p-2 bg-gray-50 rounded-lg">
                  <label className="text-[10px] font-bold text-gray-400 uppercase">Blood</label>
                  {editMode === 'medical' ? (
              ...
              <div>
                <label className="text-xs font-bold text-gray-400 uppercase">Anemia Status</label>
                {editMode === 'medical' ? (
                  <select
              ...
              <div>
                <label className="text-xs font-bold text-gray-400 uppercase">Medical History</label>
                {editMode === 'medical' ? (
                  <textarea
                    className="w-full mt-1 p-2 border rounded-lg text-sm"
                  ...
                  <p className="text-sm text-gray-800 mt-1">{patient.medicalHistory || 'None recorded'}</p>
                )}
              </div>

              <div>
                <label className="text-xs font-bold text-gray-400 uppercase">Current Medications</label>
                {editMode === 'medical' ? (
                  <textarea
                    className="w-full mt-1 p-2 border rounded-lg text-sm"
                  ...
                  <p className="text-sm text-gray-800 mt-1">{patient.currentMedications || 'None'}</p>
                )}
              </div>

              <div>
                <label className="text-xs font-bold text-gray-400 uppercase">Complications</label>
                {editMode === 'medical' ? (
                  <textarea
        ...
        {activeTab === 'family' && (
          <section className="space-y-4">
            ...
            {(isAddingChild || editingChildId) && (
              <div className="bg-gray-100 p-4 rounded-xl space-y-3 border border-gray-200 animate-in fade-in slide-in-from-top-2">
                <h4 className="text-sm font-bold text-gray-700">{editingChildId ? 'Edit Child' : 'New Child Details'}</h4>
                <input
                  type="text"
            ...
            <div className="space-y-3">
              {patientChildren.length === 0 ? (
                <p className="text-gray-400 text-sm text-center py-4">No children recorded.</p>
              ) : (
                    ...
                    <div className="bg-white p-4 rounded-xl border border-gray-100 flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">DOB: {child.dob} • {child.bloodGroup || 'N/A'}</p>
                        </div>
                      </div>
                      <button
                        onClick={() => {
        ...
        {activeTab === 'pregnancy' && patient.userType === 'pregnant' && (
          <section className="space-y-4">
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200">
              <h4 className="font-bold text-gray-700 mb-3 text-sm">Key Dates</h4>
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center p-3 bg-brand-wash rounded-lg">
                  <span className="text-sm text-brand-dark">Next ANC Visit</span>
            ...
            {/* ANC Checklist */}
            <div className="bg-white p-4 rounded-xl border border-gray-200">
              <h4 className="font-bold text-gray-700 mb-3 text-sm">ANC Checklist</h4>
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg">
                  ...
                  <span className="text-sm text-gray-600">Blood test - {patient.bloodGroup ? 'Done' : 'Pending'}</span>
                </div>
                <div className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg">
                  <HeartPulse size={16} className="text-gray-400" />
                  <span className="text-sm text-gray-600">Weight monitoring</span>
                </div>
                <div className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg">
                  <Activity size={16} className="text-gray-400" />
                  <span className="text-sm text-gray-600">TT Vaccination</span>
                </div>
                <div className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg">
                  <Activity size={16} className="text-gray-400" />
                  <span className="text-sm text-gray-600">IFA tablets distributed</span>
                </div>
              </div>
            </div>
        ...
        {activeTab === 'schemes' && (
          <section>
            <h3 className="font-bold text-gray-800 mb-3">{t('asha.eligible_schemes')}</h3>
            <div className="space-y-3">
              {schemes.map(scheme => {
                const isEnrolled = enrollments.some(e => e.schemeId === scheme.id && e.beneficiaryId === realId);
                return (
                  <div key={scheme.id} className="bg-white p-4 rounded-xl border border-gray-100 flex gap-3">
                    <img src={scheme.heroImage} alt="" className="w-16 h-16 rounded-lg object-cover" />
                    <div className="flex-1">
                      <h4 className="font-bold text-gray-900 text-sm">{scheme.title}</h4>
                      <p className="text-xs text-gray-500 line-clamp-1 mb-2">{scheme.description}</p>
