...
interface InfoSheetProps {
  ...
  sections: Section[];
  themeColor?: string; // Tailwind class like 'bg-brand'
}
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center sm:items-end sm:justify-center">
          ...
          <motion.div 
            ...
            className="relative w-full max-w-md bg-white dark:bg-gray-900 h-[90vh] sm:h-auto sm:max-h-[85vh] sm:rounded-t-3xl rounded-t-[2.5rem] shadow-2xl overflow-hidden flex flex-col"
          >
            {/* Header / Close */}
            ...
            <div className="overflow-y-auto flex-1 no-scrollbar">
              ...
              <div className="px-6 pb-12 space-y-4">
                {sections.map((section, idx) => (
                  <div key={idx} className="bg-gray-50 dark:bg-gray-800/50 p-5 rounded-2xl border border-gray-100 dark:border-gray-800">
                    <div className="flex items-center justify-between mb-2">
                      <h3 className="font-bold text-lg text-gray-900 dark:text-white">{section.title}</h3>
                      {section.badge && (
                        <span className="px-2 py-1 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 text-xs font-bold rounded-md">
                          {section.badge}
                        </span>
                      )}
                    </div>
