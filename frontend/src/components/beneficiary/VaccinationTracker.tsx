    ...
    <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
      ...
      >
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-brand-wash rounded-full flex items-center justify-center text-brand-dark">
            <Syringe size={20} />
          </div>
          <div className="text-left">
            <h4 className="font-bold text-gray-900">Vaccinations</h4>
          ...
          >
            <div className="p-4 space-y-3 bg-white">
              {VACCINE_SCHEDULE.map(vaccine => {
                ...
                return (
                  <div key={vaccine.id} className="flex items-center justify-between p-3 rounded-xl border border-gray-100 hover:border-gray-200 transition-colors">
                    <div className="flex items-center gap-3">
                      <button 
                        onClick={() => toggleVaccine(vaccine.id)}
                        className={`transition-colors ${
                          status === 'completed' ? 'text-green-500' : 'text-gray-300 hover:text-green-400'
                        }`}
                      >
                        {status === 'completed' ? <CheckCircle2 size={24} className="fill-current" /> : <Circle size={24} />}
                      </button>
                      <div>
                        <p className={`font-bold text-sm ${status === 'completed' ? 'text-gray-400 line-through' : 'text-gray-800'}`}>
                          {vaccine.name}
                        </p>
                        <p className="text-xs text-gray-500">
