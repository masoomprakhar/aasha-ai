    ...
    <div className="bg-white dark:bg-gray-900 rounded-3xl p-6 shadow-sm border border-gray-100 dark:border-gray-800">
      ...
      <div className="grid grid-cols-7 gap-y-4 gap-x-2 text-center">
        {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day, i) => (
          <div key={`${day}-${i}`} className="text-xs font-bold text-gray-400">{day}</div>
        ))}
          ...
          return (
            <button
              ...
              className={clsx(
                ...
                isSelected ? "bg-brand text-white shadow-lg shadow-rose-500/30" : "hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-300",
                isDayToday && !isSelected && "border border-rose-500 text-brand"
              )}
            >
              {format(day, 'd')}
