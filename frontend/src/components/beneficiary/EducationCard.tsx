  ...
  return (
    <>
      ...
      >
        ...
        <div>
          <h3 className="font-bold text-gray-800 leading-tight">{topic.title}</h3>
          <p className="text-xs text-gray-600 font-medium mt-1">{topic.subtitle}</p>
        </div>
      </motion.button>
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            ...
            >
              ...
              <div className="p-6">
                <ul className="space-y-4">
                  {topic.content.map((item, idx) => (
                    <li key={idx} className="flex items-start gap-3">
                      <span className="w-6 h-6 rounded-full bg-brand-soft/40 text-brand-dark flex items-center justify-center text-sm font-bold shrink-0 mt-0.5">
                        {idx + 1}
                      </span>
                ...
                <button 
                  onClick={() => setIsOpen(false)}
                  className="w-full mt-8 bg-gray-900 text-white py-4 rounded-xl font-bold text-lg"
                >
                  Okay, I understand
                </button>
              </div>
