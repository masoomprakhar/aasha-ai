...
export function BarChartWidget({ title, unit, data, colorTheme, goal }: BarChartWidgetProps) {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-3xl p-5 shadow-sm border border-gray-100 dark:border-gray-800 mb-4">
      {/* Header */}
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-bold text-lg text-gray-900 dark:text-white">
          {title} <span className="text-sm text-gray-500 font-normal">({unit})</span>
        </h3>
        <div className="flex gap-1 bg-gray-100 dark:bg-gray-800 p-1 rounded-lg">
          <button className={clsx("p-1.5 rounded-md text-white shadow-sm", theme.icon)}>
            <BarChart2 size={16} />
          </button>
          <button className="p-1.5 rounded-md text-gray-400 hover:bg-white dark:hover:bg-gray-700">
            <List size={16} />
          </button>
        </div>
      </div>
      ...
      {/* Date Nav */}
      <div className="flex items-center justify-center gap-4 mb-6 text-sm font-bold text-gray-800 dark:text-gray-200">
      ...
      <div className="relative h-48 flex items-end justify-between px-2 gap-2">
        ...
        <div className="absolute left-0 top-0 bottom-8 flex flex-col justify-between text-xs text-gray-400 w-6">
          ...
          {data.map((d) => {
            ...
            return (
              <div key={d.day} className="flex flex-col items-center gap-2 w-full group cursor-pointer" onClick={() => setSelectedDay(d.day)}>
                ...
                <div className="relative w-full flex justify-center h-full items-end">
                   {isActive && (
                    <motion.div 
                      ...
                      className={clsx(
                        "absolute -top-12 border-2 bg-white dark:bg-gray-800 rounded-full w-12 h-12 flex flex-col items-center justify-center shadow-md z-20",
                        theme.text,
                    ...
                    >
                      ...
                      {/* Triangle */}
                      <div className={clsx(
                        "absolute -bottom-1.5 w-3 h-3 bg-white dark:bg-gray-800 border-b-2 border-r-2 rotate-45",
                         colorTheme === 'green' ? 'border-[#8BC34A]' : colorTheme === 'orange' ? 'border-[#FF9800]' : 'border-[#2196F3]'
                      )}></div>
                    </motion.div>
                  )}
                ...
                {/* X-Axis Label */}
                <span className={clsx("text-xs font-medium", isActive ? "text-gray-900 dark:text-white font-bold" : "text-gray-400")}>
                  {d.day}
                </span>
              </div>
            );
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-3xl p-5 shadow-sm border border-gray-100 dark:border-gray-800 mb-4">
      <div className="flex items-center gap-2 mb-6">
        <h3 className="font-bold text-lg text-gray-900 dark:text-white">BMI</h3>
      ...
      <div className="relative h-48 w-full flex justify-center overflow-hidden">
        ...
        <div className="absolute bottom-0 text-center">
          <h2 className="text-4xl font-bold text-gray-900 dark:text-white">{bmi.toFixed(1)}</h2>
          <p className="text-xs text-gray-500">BMI (kg/m2)</p>
        </div>
      </div>
        ...
        {ranges.map((r, i) => (
          <div key={i} className="flex justify-between items-center text-sm">
            <div className="flex items-center gap-2">
              ...
              <span className={clsx("text-gray-600 dark:text-gray-400", r.active && "font-bold text-gray-900 dark:text-white")}>
                {r.label}
              </span>
            </div>
            <span className={clsx("text-gray-500", r.active && "font-bold text-gray-900 dark:text-white")}>
              BMI {r.range}
            </span>
          </div>
        ))}
