      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          <motion.div 
            ...
            className="relative w-full max-w-lg bg-white dark:bg-gray-900 rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col"
          >
            <div className="p-4 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center">
              <div>
                <h2 className="text-xl font-bold text-gray-800 dark:text-white">{t('tracker.health_log')}</h2>
                <p className="text-sm text-gray-500">{format(date, 'EEEE, d MMMM')}</p>
              </div>
            ...
            <div className="p-6 overflow-y-auto space-y-8">
              <section>
                <h3 className="text-sm font-bold text-gray-500 uppercase mb-3">{t('tracker.mood')}</h3>
                    ...
                    return (
                      <button
                        ...
                        className={`flex-1 flex flex-col items-center gap-2 p-3 rounded-2xl border-2 transition-all ${
                          isSelected ? m.color : 'border-gray-100 dark:border-gray-800 text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800'
                        }`}
                      >
                        <Icon size={24} />
              ...
              <section>
                <h3 className="text-sm font-bold text-gray-500 uppercase mb-3">{t('tracker.symptoms')}</h3>
              ...
              <section>
                <h3 className="text-sm font-bold text-gray-500 uppercase mb-3">{t('tracker.voice_log')}</h3>
                <VoiceInput onResult={handleVoiceResult} label={t('tracker.tap_speak')} />
              </section>

              <section>
                <h3 className="text-sm font-bold text-gray-500 uppercase mb-3">{t('tracker.notes')}</h3>
                <textarea
                  value={notes}
                  onChange={(e) => setNotes(e.target.value)}
                  placeholder="Anything else you want to note?"
                  className="w-full p-4 rounded-xl bg-gray-50 dark:bg-gray-800 border-none focus:ring-2 focus:ring-rose-500 min-h-[100px]"
                />
              </section>
            </div>

            <div className="p-4 border-t border-gray-100 dark:border-gray-800">
              <Button onClick={handleSave} className="w-full bg-brand-dark hover:bg-brand-dark py-4 text-lg">
