...
const THEME_COLORS = [
  { label: 'Rose', value: 'bg-brand' },
  { label: 'Teal', value: 'bg-emerald-600' },
  { label: 'Indigo', value: 'bg-indigo-600' },
  { label: 'Orange', value: 'bg-orange-500' },
  { label: 'Purple', value: 'bg-purple-600' },
  { label: 'Slate', value: 'bg-gray-800' },
];
  ...
  const renderStep1 = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <label className="block text-xs font-bold text-gray-500 uppercase mb-2">{t('partner.campaign_title')}</label>
          <input 
            type="text" 
            className="w-full p-4 rounded-xl border border-gray-200 focus:ring-2 focus:ring-brand outline-none font-bold text-lg"
        ...
        <div>
          <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Provider Organization</label>
          <select 
            className="w-full p-4 rounded-xl border border-gray-200 bg-white font-medium"
      ...
      <div>
        <label className="block text-xs font-bold text-gray-500 uppercase mb-2">{t('partner.description')}</label>
        <textarea 
          className="w-full p-4 rounded-xl border border-gray-200 h-32 focus:ring-2 focus:ring-brand outline-none"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div>
          ...
          <div className="relative">
            <Layout className="absolute left-4 top-4 text-gray-400 w-5 h-5" />
            <select 
              className="w-full pl-12 pr-4 py-4 rounded-xl border border-gray-200 bg-white appearance-none"
        ...
        <div>
          <label className="block text-xs font-bold text-gray-500 uppercase mb-2">{t('partner.budget')}</label>
          <div className="relative">
        ...
        <div>
          <label className="block text-xs font-bold text-gray-500 uppercase mb-2">{t('partner.start_date')}</label>
          <div className="relative">
      ...
      <div>
        ...
        <div className="flex gap-4">
          <div className="flex-1 relative">
            <ImageIcon className="absolute left-4 top-4 text-gray-400 w-5 h-5" />
            <input 
              type="text" 
              className="w-full pl-12 pr-4 py-4 rounded-xl border border-gray-200"
          ...
          <div className="w-32 h-14 rounded-xl bg-gray-100 overflow-hidden shrink-0 border border-gray-200">
            {formData.heroImage ? (
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
      ...
      <section>
        <h3 className="text-sm font-bold text-gray-900 mb-4 flex items-center gap-2">
          <Target className="w-5 h-5 text-brand" /> Beneficiary Type
        </h3>
            ...
            <button
              ...
              className={`p-4 rounded-2xl border-2 text-left transition-all ${
                formData.targetAudience?.userTypes?.includes(type.id as any)
                  ? 'border-indigo-600 bg-brand-wash'
                  : 'border-gray-100 bg-white hover:border-gray-200'
              }`}
            >
              <div className={`w-6 h-6 rounded-full border-2 mb-3 flex items-center justify-center ${
                 formData.targetAudience?.userTypes?.includes(type.id as any) ? 'border-indigo-600 bg-indigo-600' : 'border-gray-300'
              }`}>
      ...
      {/* Risk Level */}
      <section>
        <h3 className="text-sm font-bold text-gray-900 mb-4">Health Risk Level</h3>
            ...
            <button
              ...
              className={`px-6 py-3 rounded-xl font-bold capitalize border-2 transition-all ${
                formData.targetAudience?.riskLevel?.includes(level)
                  ? 'border-rose-500 bg-brand-wash text-rose-700'
      ...
      {/* Economic Status */}
      <section>
        <h3 className="text-sm font-bold text-gray-900 mb-4">Economic Status</h3>
        <div className="flex gap-3">
          {['bpl', 'apl'].map(status => (
            <button
              key={status}
  ...
  const renderStep3 = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
      <GlassCard className="p-6">
        <h3 className="text-sm font-bold text-gray-500 uppercase mb-4">Theme Color</h3>
        <div className="flex gap-4">
          {THEME_COLORS.map(color => (
            <button
              ...
              className={`w-12 h-12 rounded-full ${color.value} transition-transform hover:scale-110 ring-2 ring-offset-2 ${
                formData.micrositeConfig?.themeColor === color.value ? 'ring-gray-900' : 'ring-transparent'
              }`}
              title={color.label}
            />
          ))}
      ...
      <GlassCard className="p-6">
        <h3 className="text-sm font-bold text-gray-500 uppercase mb-4">About Section</h3>
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
      ...
      <GlassCard className="p-6">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-sm font-bold text-gray-500 uppercase">Beneficiary To-Do List</h3>
          <button 
            onClick={() => {
              const tasks = formData.micrositeConfig?.tasks || [];
              updateMicrosite('tasks', [...tasks, { id: Date.now().toString(), title: '', isMandatory: false }]);
            }}
            className="text-xs font-bold text-brand flex items-center gap-1 bg-brand-wash px-3 py-1.5 rounded-lg hover:bg-indigo-100"
          >
          ...
          {formData.micrositeConfig?.tasks.map((task, idx) => (
            <div key={idx} className="flex gap-3 items-start p-3 bg-gray-50 rounded-xl border border-gray-100">
              <div className="flex-1 space-y-2">
                <input 
                  type="text" 
                  className="w-full p-2 bg-white rounded-lg border border-gray-200 text-sm font-bold"
              ...
              <div className="flex flex-col gap-2">
                <button 
                  ...
                  className={`p-2 rounded-lg transition-colors ${task.isMandatory ? 'bg-red-100 text-red-600' : 'bg-gray-200 text-gray-400'}`}
                ...
                <button 
                  ...
                  className="p-2 rounded-lg bg-gray-200 text-gray-500 hover:bg-red-100 hover:text-red-500"
                >
                  <Trash2 size={16} />
                </button>
              </div>
      ...
      <GlassCard className="p-6">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-sm font-bold text-gray-500 uppercase">Data Collection Form</h3>
          <button 
            onClick={() => {
        ...
        <div className="space-y-3">
          {formData.micrositeConfig?.customFormFields.map((field, idx) => (
            <div key={idx} className="flex gap-3 items-center p-3 bg-gray-50 rounded-xl border border-gray-100">
              <input 
                type="text" 
                className="flex-1 p-2 bg-white rounded-lg border border-gray-200 text-sm font-bold"
              ...
              />
              <select 
                className="p-2 bg-white rounded-lg border border-gray-200 text-xs font-medium"
                value={field.type}
                onChange={e => {
                  const newFields = [...(formData.micrositeConfig?.customFormFields || [])];
    ...
    <div className="flex flex-col lg:flex-row gap-8 h-full">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        ...
        <div className="flex items-center gap-2 mb-6 px-1">
          {[1, 2, 3, 4].map(s => (
            <div key={s} className={`h-2 rounded-full transition-all duration-500 ${s <= step ? 'w-12 bg-gray-900' : 'w-4 bg-gray-200'}`} />
          ))}
          <span className="ml-auto text-xs font-bold text-gray-400">Step {step} of 4</span>
        </div>
        ...
        {/* Navigation Buttons */}
        <div className="pt-6 border-t border-gray-100 flex justify-between items-center bg-gray-50 sticky bottom-0 z-10">
          {step > 1 ? (
            <button 
              onClick={handleBack}
          ...
          {step < 4 ? (
            <button 
              onClick={handleNext}
              className="px-8 py-3 rounded-xl font-bold bg-gray-900 text-white hover:bg-gray-800 transition-colors flex items-center gap-2 shadow-lg"
            >
            ...
            <div className="flex gap-3">
              ...
              <button 
                ...
                disabled={isLoading}
                className="px-8 py-3 rounded-xl font-bold bg-indigo-600 text-white hover:bg-indigo-700 transition-colors flex items-center gap-2 shadow-lg shadow-brand-ring"
              >
      ...
      {/* Right: Preview */}
      <div className="hidden lg:block w-[380px] shrink-0 bg-gray-100 rounded-3xl p-8 border border-gray-200 h-full overflow-hidden">
        <div className="text-center mb-6">
          <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest flex items-center justify-center gap-2">
