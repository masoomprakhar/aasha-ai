...
export default function PhonePreview({ scheme }: Props) {
  ...
  const themeColor = config?.themeColor || 'bg-brand';
  
  // Extract color for text/borders based on bg class
  const getTextColor = (bgClass: string) => {
    if (bgClass.includes('rose')) return 'text-brand-dark';
    if (bgClass.includes('teal')) return 'text-brand-dark';
    if (bgClass.includes('indigo')) return 'text-brand';
    if (bgClass.includes('orange')) return 'text-orange-600';
    if (bgClass.includes('purple')) return 'text-purple-600';
    if (bgClass.includes('slate')) return 'text-gray-900';
  ...
  return (
    <div className="w-[320px] h-[640px] bg-gray-900 rounded-[3rem] p-3 shadow-2xl border-4 border-gray-800 relative mx-auto select-none overflow-hidden ring-4 ring-gray-200">
      {/* Notch */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-32 h-6 bg-gray-900 rounded-b-xl z-30"></div>
      
      {/* Screen Content */}
      <div className="w-full h-full bg-gray-50 rounded-3xl overflow-hidden flex flex-col relative font-sans">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-5 no-scrollbar pb-24">
          ...
          <div className="flex flex-wrap gap-1">
            {scheme.targetAudience?.userTypes?.map(t => (
              <span key={t} className="text-[9px] font-bold uppercase px-2 py-1 bg-gray-200 text-gray-600 rounded-md">
                {t}
              </span>
            ))}
          ...
          {config?.tasks && config.tasks.length > 0 && (
            <div>
              <h4 className="font-bold text-xs text-gray-400 uppercase mb-3 tracking-wider ml-1">Your Tasks</h4>
              <div className="space-y-2">
                {config.tasks.map((task, i) => (
                  <div key={i} className="bg-white p-3 rounded-xl border border-gray-100 flex items-start gap-3 shadow-sm">
                    ...
                    <div>
                      <span className="text-xs font-bold text-gray-800 block">{task.title}</span>
                      {task.description && <span className="text-[10px] text-gray-500 block mt-0.5">{task.description}</span>}
                      {task.isMandatory && (
          ...
          {config?.customFormFields && config.customFormFields.length > 0 && (
            <div>
              <h4 className="font-bold text-xs text-gray-400 uppercase mb-3 tracking-wider ml-1">Registration Details</h4>
              <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 space-y-3">
                {config.customFormFields.map((field, i) => (
                  <div key={i}>
                    ...
                    {field.type === 'file' ? (
                      <div className="w-full p-3 border border-dashed border-gray-300 rounded-xl flex items-center justify-center text-gray-400 gap-2 text-xs bg-gray-50">
                        <Upload size={14} /> Upload Document
                      </div>
                    ) : field.type === 'date' ? (
                      <div className="w-full p-2.5 border border-gray-200 rounded-xl flex items-center text-gray-400 gap-2 text-xs bg-gray-50">
                        <Calendar size={14} /> Select Date
                      </div>
                    ) : (
                      <div className="w-full p-2.5 border border-gray-200 rounded-xl bg-gray-50 h-9"></div>
                    )}
                  </div>
                ))}
              </div>
        ...
        {/* Bottom Action */}
        <div className="p-4 bg-white border-t border-gray-100 shrink-0 absolute bottom-0 left-0 right-0 z-20">
