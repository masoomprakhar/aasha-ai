  ...
  persist(
    (set, get) => ({
      ...
      fetchInitialData: async (force = false) => {
        ...
        try {
          const isDevBypass = tokenManager.getAccessToken() === BYPASS_TOKEN;
      ...
      ensureBeneficiaryProfile: async (userId, name) => {
        ...
        if (!exists) {
          if (tokenManager.getAccessToken() === BYPASS_TOKEN) {
            const localProfile: BeneficiaryProfile = {
              id: uuidv4(),
              userId,
              name,
              userType: 'girl',
            ...
            set((state) => ({
              beneficiaries: [...state.beneficiaries, localProfile],
            }));
            return;
          }
