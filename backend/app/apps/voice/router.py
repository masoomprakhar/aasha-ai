...
from app.apps.ai.language_utils import normalize_language, resolve_response_language, detect_language_from_text
...
async def transcribe_audio(
    ...
    lang_hint = normalize_language(language, default="auto") if language else "auto"
    whisper_language = None if lang_hint == "auto" else lang_hint

    # Prefer OpenAI Whisper API when configured
    if openai_service.is_configured:
        try:
            print("[Transcribe] Using OpenAI whisper-1 API...")
            ...
            detected = normalize_language(
                result.get("language") or detect_language_from_text(transcript),
                default="hi",
            )
            ...
            print(f"[Transcribe] OpenAI API error: {e}")
            raise HTTPException(
                status_code=500,
...
async def chat_with_asha_didi(
    ...
    # Normalize language and auto-detect from user message
    preferred = normalize_language(request.language)
    request.language = resolve_response_language(request.message, preferred)
...
async def text_to_speech(
    ...
    if not openai_service.is_configured:
        raise HTTPException(
            status_code=503,
            detail="OpenAI TTS is not configured. Set OPENAI_API_KEY in backend/.env",
        )

    preferred = normalize_language(request.language)
    tts_lang = detect_language_from_text(request.text.strip(), preferred)

    try:
        audio_bytes = await openai_service.synthesize_speech(
