...
from app.apps.ai.openai_service import openai_service
from app.apps.ai.language_utils import (
...
class GeminiService:
    ...
    def __init__(self):
        # Normalize: the hosted service exposes POST /generate. Tolerate an env
        # value that points at the bare host (a common misconfiguration).
        url = settings.GEMINI_API_URL.rstrip('/')
        if not url.endswith('/generate'):
            url = f"{url}/generate"
        self.api_url = url
    
    async def generate(self, prompt: str) -> AIResponse:
        """Generate a response using OpenAI (preferred) or Gemini fallback."""
        if openai_service.is_configured:
            return await openai_service.generate(prompt)

        return await self._generate_gemini(prompt)

    async def _generate_gemini(self, prompt: str) -> AIResponse:
        """Call the hosted Gemini API to generate a response."""
        try:
    ...
    async def chat_with_asha_didi(
        ...
        response_lang = resolve_response_language(user_message, language)
        ...
        # Build system prompt based on resolved response language
        if response_lang == 'hi':
...
महत्वपूर्ण: उपयोगकर्ता हिंदी में बोले/लिखे तो केवल हिंदी में जवाब दें। अंग्रेज़ी शब्दों से बचें (सिर्फ "ASHA", "Red Zone" जैसे ज़रूरी शब्द छोड़कर)।"""
        else:
...
IMPORTANT: If the user writes or speaks in English, reply only in English. Use familiar terms like "didi", "behan" only when natural."""
        ...
        try:
            if openai_service.is_configured:
                ai_message = await openai_service.chat_completion(messages)
            else:
                # Gemini fallback via single prompt
    ...
    async def get_health_guidance(self, query: str, language: str = "hi") -> str:
        ...
        response_lang = resolve_response_language(query, language)
