Revamp my existing frontend with a React + TypeScript + Tailwind CSS using Vite. The entire code lives in `frontend` folder. No extra libraries beyond `react`, `react-dom`, `lucide-react`, and Tailwind.
...
Do not add any other sections, no Supabase wiring, no routing. Only the revamp of existing frontend matching as the website page described above.